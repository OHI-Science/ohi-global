...
#   
#   # check tonnes
#   fao_tonnes = read.csv(file.path(dir_d, 'data', 'FAO-Commodities_v2011_tonnes_lyr.csv')); head(fao_tonnes); 
#   fao_usd    = read.csv(file.path(dir_d, 'data', 'FAO-Commodities_v2011_usd_lyr.csv')); head(fao_usd)
#   
...
#          usd_lyr   , usd,    usd_diff)
# 
# tbx_diff = np_tbx_c %>%  
#   filter(tonnes_diff != 0 | usd_diff != 0) %>%
#   filter(!is.na(tonnes_diff) | !is.na(usd_diff)) # I'm getting an error here I can't get around...
# head(tbx_diff) 
