...
data <- read.csv('https://raw.githubusercontent.com/OHI-Science/ohiprep/master/globalprep/np/v2020/raw/commodities2products_new.csv') %>%
  group_by(product) %>%
...
```{r np commodity, echo=FALSE, message=FALSE, warning=FALSE, error=FALSE}
data <- read.csv('https://raw.githubusercontent.com/OHI-Science/ohiprep/master/globalprep/np/v2020/raw/commodities2products_weighting.csv') %>%
  group_by(product) %>%
  summarize(subcategory = paste(commodity, collapse = ", ")) %>%
  select(commodity = product, subcategory) %>%
  mutate(commodity = gsub("_", " ", commodity))
