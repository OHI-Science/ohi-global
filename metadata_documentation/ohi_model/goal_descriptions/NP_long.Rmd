...
In many countries the harvest of non-food natural products is important for local economies and can also be traded internationally. The sustainable harvest of these products is therefore an important component of a healthy ocean. This goal assesses the ability of countries to maximize the sustainable harvest of living marine resources, and includes three natural product categories: ornamental fish, fish oil and fish meal, and inedible seaweeds and marine plants (Table 5.10). It does not include bioprospecting which focuses on potential (and largely unknowable and potentially infinite) value rather than current realized value, or non-living products such as oil and gas or mining products which by definition are not sustainable.   We excluded natural products with no global data (e.g., wood from mangroves), and others that are widely seen as (currently) unsustainably harvested, such as oils from mammals and coral. In contrast to previous years, we also excluded shells and sponges due to the very low impact in most countreis and their highly variable contribution to economies over time, which leads to instability in scores. 

#### Current status

To determine the total production in tonnes for seaweed we summed seaweed production provided in the FAO global aquaculture production data [@united_nations_fao_2018]. To determine total production in tonnes of ornamental fish we summed the exported products provided in the FAO commodities data [@united_nations_fao_2017]. Finally, to determine the total production in tonnes for fish oil and fish meal, we used Watson (2019) global marine fisheries catch data [@watson_global_2019] and B/Bmsy data [@ricard_examining_2012; @costello_status_2012; @martell_simple_2013; @thorson_new_2013; @rosenberg_developing_2014; @costello_global_2016] calculated from our fisheries sub-goal (methods can be found in section 6.6.1), and subsetted the stocks for stocks used for fish oil production (Table 5.10) [@froehlich_avoiding_2018]. The tonnes of harvest of fish oil and fish meal species were multiplied by 0.7 to reflect the amount of the fish actually going to production of feed or oil. 

**Table 5.11. Natural product categories**. 
...
data <- read.csv('https://raw.githubusercontent.com/OHI-Science/ohiprep_v2020/gh-pages/globalprep/np/v2020/raw/commodities2products_new.csv') %>%
  ...
  select(Commodity = commodity, Subcategory = subcategory)
...
Because dollar values were not present in the data that was used for seaweeds and fish oil, we relied on the FAO global commodities dataset to obtain these, which we used to calculate $w_c$ for each category (Table 5.11).
