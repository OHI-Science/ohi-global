...
You will be using the OHI Toolbox software to do this, and follow the instructions here (`OHI_Instructions.Rmd`). Walk through line-by-line and execute the code in R. You will turn in a Word document with figures visualizing the changes you've made for a country of your choice; instructions for what to turn in are below in Section 3. 
...
Since we're in RStudio with a `.Rproj` file, our current working directory is `wherever/you/saved/it/ohi-global`. All of our work for the assignment happens in the `ESM_270` folder; this file is `ohi-global/ESM_270/ESM_270_OHI.Rmd`. Let's change the working directory to the ESM_270 folder, and install the packages we need to get going. 
...
## re-calculate scores
readr::write_csv(conf_new_wt, 'conf/goals.csv') # use new weight
source('calculate_scores.r')
...
readr::write_csv(conf_orig, 'conf/goals.csv') # use original conf
source('calculate_scores.r')
...
Turn in a simple document describing: 

- the new goal weight
- the new reference point
- how these changes affected the final scores
- what this means for scoring ocean health

Turn in your final assignment as a Word document. You can do this on your own or using a template we've started, right here in R! You can modify `ESM_270/OHI_Assignment.Rmd` press the `Knit Word` document to create a Word document like `ESM_270/OHI_Assignment.docx` right from R. You'll need to modify the `.Rmd` file with your figure names and write up your comments and reactions to the lab before knitting.
