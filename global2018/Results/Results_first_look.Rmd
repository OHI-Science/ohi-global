---
title: 'OHI 2018: Summary of results'
...
This document describes results from the OHI 2018 global assessment.  This is mostly an internal document to explore and check results.  It also creates the figures used on the website and in other documents.
```{r control, echo=FALSE, error=FALSE, warning=FALSE, include = FALSE}
...
dateFile = '2018-10-22' #date extension on data files used for all tables/figures

## General settings to control
scenario <- 2018 #identify scenario of focus (this can be changed to obtain data for other years)
benchmark = 2017  # previous year that is used for old vs. new OHI analyses
oldCommit = 'e0ed46b5988bb108faa7ae8bc839ba9a9f2bcf12' # sha final commit for 2017 assessment
saveFile <- 'global2018' #location where files that are created are to be saved
...
ohi_final <- read.csv(here("eez/scores.csv"))
write.csv(ohi_final, here(sprintf("%s/scores_%s.csv", saveFile, Sys.Date())), row.names=FALSE)
```

```{r final_data_modified, eval=update_data, include=FALSE}

# if data are updated, this saves a modified version that is easier to read
ohi_final <- read.csv(here(sprintf("%s/scores_%s.csv", saveFile, dateFile)))

# User friendly version of data: region names are also added to data and full goal names.
ohi_final_formatted <- read.csv(here(sprintf("%s/scores_%s.csv", saveFile, dateFile))) %>%
...
write.csv(ohi_final_formatted, here(sprintf("%s/OHI_final_formatted_scores_%s.csv", saveFile, dateFile)), row.names=FALSE)

## create a version of data with region index scores wide
ohi_final_wide <- read.csv(here(sprintf("%s/OHI_final_formatted_scores_%s.csv", saveFile, dateFile))) %>%
...
write.csv(ohi_final_wide, here(sprintf("%s/Results/data/index_scores_wide_all_years.csv", saveFile)), row.names=FALSE)
...
index_avg <- read.csv(here(sprintf('%s/OHI_final_formatted_scores_%s.csv', saveFile, dateFile)), stringsAsFactors = FALSE) %>%
  filter(dimension == "score") %>%
...
write.csv(index_avg_spread, here(sprintf("%s/Results/data/avg_goal_scores.csv", saveFile)), row.names=FALSE)
...
data_all <- read.csv(here(sprintf("%s/OHI_final_formatted_scores_%s.csv", saveFile, dateFile)))
...
write.csv(data_final, here(sprintf("%s/Results/data/scores_eez%s.csv", saveFile, year)), row.names = FALSE, na='')
...
[These data](https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/global2018/OHI_final_formatted_scores_2018-10-22.csv) are the OHI scores for the eez regions of 220 countries and territories from 2012 to 2018.
...
source(here(sprintf('%s/Results/functions/flowerPlot.R', saveFile)))

PlotFlower(
  region_plot     = NA,
  dir_fig_save = here(sprintf("%s/Results/figures/FlowerPlots", saveFile)),
  assessment_name = "Global average",
  year_plot = 2018,
  scores_file = here(sprintf("%s/scores_%s.csv", saveFile, dateFile)),
  scenario_folder = "eez"
)
...
source(here('global2016/Reporting/map_fxns.R'))
...
if(year != scenario){
  if(sum(grepl(sprintf("year_%s", year), 
               list.files(here(sprintf("%s/Results/figures/maps_by_goal_mol", saveFile))))) == 0){
  dir.create(here(sprintf("%s/Results/figures/maps_by_goal_mol/year_%s", saveFile, year)))
  }
  }
    
scores_df <- read.csv(here(sprintf('%s/Results/data/scores_eez%s.csv', saveFile, year)), stringsAsFactors = FALSE) %>%
...
for (fld in mapFlds) { # fld <- mapFlds[1]
  ...
  if(year==scenario){
   fig_save = here(sprintf('%s/Results/figures/maps_by_goal_%s/global_map_%s_%s_%s.png', saveFile, prj, fld, year, prj))
} else{ 
    fig_save = here(sprintf('%s/Results/figures/maps_by_goal_%s/year_%s/global_map_%s_%s_%s.png', saveFile, prj, year, fld, year, prj))
    }
...
dir.create(here(sprintf('%s/Results/figures/maps_by_goal_%s/year_%s_nolegend', saveFile, prj, year)))
...
for (fld in mapFlds) { # fld <- mapFlds[1]
   ...
   fig_save = here(sprintf('%s/Results/figures/maps_by_goal_%s/year_%s_nolegend/global_map_%s_%s_%s.png', saveFile, prj, year, fld, year, prj))
...
Country index scores for 2018.  Maps describing goal and index scores can be downloaded from [here](https://github.com/OHI-Science/ohi-global/tree/draft/global2018/Results/figures/maps_by_goal_mol).
...
hist_data <- read.csv(here(sprintf("%s/Results/data/scores_eez%s.csv", saveFile, year))) %>%
...
ggsave(here(sprintf("%s/Results/figures/score_histogram_color.png", saveFile)), width=4, height=3, units=c("in"), dpi=300)
...
rank_data <- read.csv(here(sprintf("%s/Results/data/scores_eez%s.csv", saveFile, year))) %>%
select(region_name, Index)
...
This carpet plot figure ([download as high resolution png](https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/global2018/Results/figures/carpetPlot.png)) provides an overview of the scores from the 2018 assessment.  Each row represents a region, the main groupings represent goals, and within each goal there are 6 years of data.  Black regions indicate no data.
...
ggplot(data, aes(y=region_name, x=scenario, fill=value)) + 
  ...
  ylab("") + 
  xlab("")
ggsave(here(sprintf("%s/Results/figures/carpetPlot.png", saveFile)), width=20, height=35)
...
## Data
This interactive table describes the index and goal scores for the regions in 2018 (or, [download csv version](https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/global2018/Results/data/scores_eez2018.csv)): 

```{r score table, message=FALSE, warning=FALSE, error=FALSE, echo=FALSE}

data_final_inlay <- read.csv(here(sprintf("%s/Results/data/scores_eez%s.csv", saveFile, year))) %>%
  select(-region_id) %>%
  data.frame()
...
hwrite(data_final, here(sprintf('%s/Results/data/scores_eez%s.html', saveFile, scenario)), br=TRUE, center=TRUE, border=0, 
...
scores <- read.csv(here(sprintf("%s/OHI_final_formatted_scores_%s.csv", saveFile, dateFile)))
...
write.csv(trends_spread, here(sprintf("%s/Results/data/trends_%s.csv", saveFile, scenario)), row.names = FALSE, na='')
...
hwrite(trends_spread, here(sprintf('%s/Results/data/trends_%s.html', saveFile, scenario)), br=TRUE, center=TRUE, border=0, 
       row.style=list(goal='text-align:center'), table.style='padding: 10px; margin:20px;', 
...
A color-coded table of trend values for each country is available [here](https://rawgit.com/OHI-Science/ohi-global/draft/global2018/Results/data/trends_2018.html) (as well as a [csv file](https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/global2018/Results/data/trends_2018.csv)).  
...
source(here(sprintf('%s/Results/functions/map_fxns_trend.R', saveFile)))
...
### get OHI data and rename column headings
scores_df <- read.csv(here(sprintf('%s/Results/data/trends_%s.csv', saveFile, scenario)), stringsAsFactors = FALSE) %>%
...
for (fld in mapFlds) { # fld <- mapFlds[1]
    ...
    fig_save = here(sprintf('%s/Results/figures/map_trends/trends_map_%s_%s.png', saveFile, fld, prj))
...
![](figures/map_trends/trends_map_Index_mol.png) 
...
ggsave(here(sprintf("%s/Results/figures/trend_histogram_color.png", saveFile)), width=5, height=4)
```
...
source(here(sprintf("%s/Results/functions/trend_barplot.R", saveFile)))

```

A heat map describing change in trend for country and goals (download png [here](figures/Trend_barplot.png)).

```{r trend barplot, echo=FALSE, fig.height=30, fig.width=10}

## Figure created in trend_barplot.R

knitr::include_graphics(here(sprintf("%s/Results/figures/Trend_barplot.png", saveFile)))
```
...
## Scores
Goal/subgoal scores for the 2018 scenario.
```{r goal histograms, message=FALSE, warning=FALSE, error=FALSE, echo=FALSE, eval=goal_hists}

data <- read.csv(here(sprintf('%s/Results/data/scores_eez%s.csv', saveFile, scenario))) %>%
  filter(region_id != 0) %>%
...
ggsave(g, filename=here(sprintf("%s/Results/figures/goal_histograms/%s_scores.png", saveFile, goal)), height=3, width=4)
}
...
data <- read.csv(here(sprintf('%s/Results/data/scores_eez%s.csv', saveFile, scenario))) %>%  filter(region_id != 0) %>%
...
## Trends
Annual change in goal/subgoal scores calculated using a linear regression on scores from 2012 to 2018. 

![](figures/map_trends/trends_map_AO_mol.png)

![](figures/map_trends/trends_map_SPP_mol.png)


![](figures/map_trends/trends_map_HAB_mol.png)

![](figures/map_trends/trends_map_CP_mol.png)

![](figures/map_trends/trends_map_CS_mol.png)

![](figures/map_trends/trends_map_CW_mol.png)

![](figures/map_trends/trends_map_FIS_mol.png)

![](figures/map_trends/trends_map_MAR_mol.png)

![](figures/map_trends/trends_map_ICO_mol.png)

![](figures/map_trends/trends_map_LSP_mol.png)

![](figures/map_trends/trends_map_NP_mol.png)

![](figures/map_trends/trends_map_TR_mol.png)
...
index <- read.csv(here(sprintf("%s/Results/data/avg_goal_scores.csv", saveFile)), stringsAsFactors = FALSE) %>%
  select(region_id, country=region_name, index_avg = Index)

trend <- read.csv(here(sprintf("%s/Results/data/trends_%s.csv", saveFile, scenario))) %>%
...
We made a few changes to some goals, such as the addition of edible seaweed species to Mariculture and removal of edible seaweed species from Natural Products. Furthermore we changed the data source used for Fisheries from SAUP to Watson.  We also had fairly large changes to how we process species data.  

```{r compare assessments, message=FALSE, warning=FALSE, error=FALSE, echo=FALSE}

data <- read.csv(here(sprintf("%s/OHI_final_formatted_scores_%s.csv", saveFile, dateFile)), stringsAsFactors = FALSE) %>%
  rename("assess2018"=value) 

data_old <- read.csv(here(sprintf('global%s/OHI_final_formatted_scores_2017-11-22.csv', benchmark)), stringsAsFactors = FALSE) %>%
  dplyr::rename("assess2017"=value)
