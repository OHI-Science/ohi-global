...
# Change over time 
This section describes global patterns in index and goal scores over time.

### Average global goal and index scores across years

(NOTE: Livelihood and economy goals are not included here)

The Global Index score for 2017 (eez area weighted average of region scores) was: 70.  The following table provides the global scores (eez area weighted average of region scores) for the Index and goals/subgoals.
...
region_trends <- tidy(region_trends, mdl) %>%
  ...
  left_join(goal_names) %>%
  filter(region_id != 0) %>%
  select(region_id, goal=long_goal, estimate, p.value) %>%
  data.frame() %>%
  group_by(goal) %>%
  summarize(num_rgns_pos_trend = sum(estimate > 0),
            num_rgns_neg_trend = sum(estimate < 0),
            mean_trend = mean(estimate, na.rm=TRUE),
            sd_trend = sd(estimate, na.rm=TRUE))
...
### Regional goal and index scores for 2017
...
# Change over time: six year trends in scores
...
The following is a comparison of the global status scores generated for 2016 by this year's assessment vs. last year's assessment. 
...
Other regiosn changed by less than 2 points.
...
This color-coded [table](https://rawgit.com/OHI-Science/ohi-global/draft/global2017/Results/data/change_in_scores_benchmark2016_eez2017.html) compares the 2015 index scores generated for each region/goal between this year's and last year's assessment.  

Because these are index scores, changes reflect updates to pressure and resilience scores as well as status.  

The following interactive plot provides an overview of how the 2016 scores changed between the 2016 and 2017 assessment for all goals and dimensions.
