...
dateFile = '2017-11-20' #date extension on data files used for all tables/figures
...
### Primary dataset
[Data](https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/global2017/OHI_final_formatted_scores_2017-11-15.csv) are the OHI scores for the eez regions of 220 countries and territories from 2012 to 2017.
...
```
This section describes global patterns in index and goal scores.  The overall global score was `data_final %>% filter(region_name == "Global average") %>% select(Index) %>% .[['Index']]`.
...
The global average decreased by an average of `index$Index` points per year.  There was a good deal of variation in trend among regions and goals.  
...
The following is a very preliminary analysis of how the global scores changed over time.  Global index scores decreased by an average `index$Index` points per year (although, not significant with this rough analysis), with `sum(trends_spread$Index<0)` regions having decreasing trends and `sum(trends_spread$Index>0)` having increasing trends. 
...
Stacked trend plot like in 2016 results
