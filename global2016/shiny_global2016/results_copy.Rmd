...
rgn_names <- read_csv(sprintf('../../eez%s/layers/rgn_global.csv', scenario)) %>%
...
### set it up so can filter by georegion.  Attach georegions to dataframe;
### if "all" is selected, color by continent; if a continent is selected,
### color by subregion.
index <- read_csv("data/scores_eez2016.csv") %>%
  select(region_id, country, Index)

vaxis <- index$Index[index$region_id==0]

trend <- read_csv("data/trends_2016.csv") %>%
...
#     geom_point(data=trend %>% filter(country %in% c(label1, label2)), aes(x=Index, y=index_trend), shape=19, size=1.75, color="darkorange") +
...
data <- read_csv(sprintf('data/radical_%s.csv', radicalFile)) 
...
trends <- read_csv(sprintf("data/trends_%s.csv", scenario)) %>%
...
hdi <- read_csv("~/github/ohiprep/globalprep/supplementary_information/v2016/HDI_data.csv") %>%
  select(rgn_id, hdi=value)

# coastal population
pop <- read_csv("~/github/ohiprep/globalprep/mar_prs_population/v2016/output/mar_pop_25mi.csv") %>%
  filter(year == 2015) %>%
  select(rgn_id, coastal_pop = popsum) %>%
  mutate(log_coastal_pop = log(coastal_pop + 1))

## chi data
chi <- read_csv("~/github/ohiprep/globalprep/supplementary_information/v2016/CHI_data.csv") %>%
  filter(sp_type == "eez") %>%
...
```{r trend analysis}
### prepare data:
data <- read_csv(sprintf('../radical_%s.csv', radicalFile)) 
...
results_lm <- tidy(data_lm, mdl) %>%
  ...
  data.frame()
# 
# weights <- read_csv("../../eez2016/layers/rgn_area.csv") %>%
...
eez_area <- read_csv("../../eez2016/layers/rgn_area.csv") %>%
  select(region_id=rgn_id, area_km2) %>%
...
rel = read_csv("../../eez2016/layers/np_harvest_tonnes_relative.csv") 
tons_proc = read_csv("../../eez2016/layers/np_harvest_tonnes.csv")

tons <- read_csv("../../../ohiprep/globalprep/np/v2016/int/tonnes.csv") %>%
  group_by(rgn_id, rgn_name, year, product) %>%
  summarize(tonnes= sum(tonnes, na.rm=TRUE)) %>%
  arrange(product, year)

usd <- read_csv("../../../ohiprep/globalprep/np/v2016/int/usd.csv") %>%
  group_by(rgn_id, rgn_name, year, product) %>%
...
tmp <- read_csv("../radical_with_region_names_2016-11-17.csv") %>%
  filter(region_id == 45) %>%
  filter(goal == "NP") %>%
...
trends <- read_csv("data/trends_2016.csv") %>%
  arrange(FIS) %>%
  select(country, region_id, FIS)

### Does status seem to be increasing?
fis_status <- read_csv("../../global2016/radical_2016-11-17.csv") %>%
  filter(region_id != 0) %>%
  filter(region_id != 213) %>%
...
fis_trend <- read_csv("../../global2016/radical_2016-11-17.csv") %>%
  filter(region_id != 0) %>%
  filter(region_id != 213) %>%
  filter(region_id <= 250) %>%
...
### Pressures
fis_pressures <- read_csv("../../global2016/radical_2016-11-17.csv") %>%
  filter(region_id != 0) %>%
  filter(region_id != 213) %>%
  filter(region_id <= 250) %>%
...
## Norway
catch <- read_csv("../../eez2016/layers/fis_meancatch.csv") %>%
  filter(rgn_id == 223) %>%
  filter(year==2010) %>%
  arrange(mean_catch)

bmsy <- read_csv("../../eez2016/layers/fis_b_bmsy.csv") %>%
  filter(rgn_id == 223) %>%
...
tmp <- read_csv("../../../ohiprep/globalprep/fis/v2016/data/fis_bbmsy_gf.csv") %>%
...
### Does status seem to be increasing?
cp_status <- read_csv("../../global2016/radical_with_region_names_2016-11-17.csv") %>%
  filter(region_id != 0) %>%
  filter(region_id != 213) %>%
  filter(region_id <= 250) %>%
  filter(dimension=="score") %>%
...
read_csv("../../global2016/radical_with_region_names_2016-11-17.csv") %>%
...
## TR goal
read_csv("../../eez2016/layers/tr_jobs_pct_tourism.csv") %>%
  filter(rgn_id==3)

read_csv("../../eez2012/layers/tr_jobs_pct_tourism.csv") %>%
  filter(rgn_id==3)

read_csv("../../eez2016/layers/tr_sustainability.csv") %>%
  filter(rgn_id==3)
read_csv("../../eez2012/layers/tr_sustainability.csv") %>%
  filter(rgn_id==3)

read_csv("../../eez2016/layers/tr_travelwarnings.csv") %>%
  filter(rgn_id==3)
```
...
data <- read_csv("data/trends_2016.csv") %>%
  filter(region_id != 0) %>%
