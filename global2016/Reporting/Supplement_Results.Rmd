...
##### Table 1. Description of 10 goals used to calculate OHI scores.
...
##### Table 2. Description of subgoals used to calculate OHI scores.
...
The data layers (section 6 in S2 Methods) and functions used to calculate scores can be downloaded from Github: [ohi-global v2016.1: Global scenarios data for Ocean Health Index](https://github.com/OHI-Science/ohi-global/releases).  
...
##### Figure 1. Map and flowerplot of 2016 OHI index scores.
The map (A) describes the overal index scores for each OHI region.  The flowerplot (B) describes the global average scores for each goal/subgoal, weighted by each region's eez area. 
...
map_label <- textGrob("A", just=19, gp=gpar(fontsize=18))
Index_flower <- rasterGrob(readPNG("figures/FlowerPlots/flower_GLOBAL_2016.png"), interpolate=TRUE)
flower_label <- textGrob("B", just=19, gp=gpar(fontsize=18))
...
##### Figure 2. Maps and histograms of 2016 OHI goal scores.
The scores for each of the 10 OHI goals (A-J) displayed for each region (left) and as a histogram (right).
...
AO_text <- textGrob("A", gp=gpar(fontsize=14), vjust=-9.5)
...
BD_text <- textGrob("B", gp=gpar(fontsize=14), vjust=-9.5)
...
CP_text <- textGrob("C", gp=gpar(fontsize=14), vjust=-9.5)
...
CS_text <- textGrob("D", gp=gpar(fontsize=14), vjust=-9.5)
...
CW_text <- textGrob("E", gp=gpar(fontsize=14), vjust=-9.5)
...
FP_text <- textGrob("F", gp=gpar(fontsize=14), vjust=-9.5)
...
LE_text <- textGrob("G", gp=gpar(fontsize=14), vjust=-9.5)
...
NP_text <- textGrob("H", gp=gpar(fontsize=14), vjust=-9.5)
...
SP_text <- textGrob("I", gp=gpar(fontsize=14), vjust=-9.5)
...
TR_text <- textGrob("J", gp=gpar(fontsize=14), vjust=-9.5)
...
##### Figure 3. Maps and histograms of 2016 OHI subgoal scores.
The subgoal scores for the following goals: biodiversity (A), foob provisioning (B), sense of place (C), and livelihoods and economies (D).  The scores are displayed as a map (left) and histogram (right).

```{r subgoal scores, echo=FALSE, fig.width=10, fig.height=25}

# BD
BD <- textGrob("A Biodiversity", gp=gpar(fontsize=12), just="left", x=0)
bd <- textGrob("")
...
## FP
FP <- textGrob("B Food provisioning", gp=gpar(fontsize=12), just="left", x=0)
fp <- textGrob("")
...
SP <- textGrob("C Sense of place", gp=gpar(fontsize=12), just="left", x=0)
sp <- textGrob("")
...
# LE
LE <- textGrob("D Livelihoods & economies", gp=gpar(fontsize=12), just="left", x=0)
le <- textGrob("")
...
##### Figure 4. Carpet plot describing scores for each region and year.
...
##### Figure 5. Average annual change in OHI Index scores.
...
##### Figure 6. Average annual change in OHI goal scores.
...
##### Figure 7. Average annual change in OHI subgoal scores.
Map of the slope estimates from a linear regression model of the 10 subgoal scores from 2012 to 2016 for each region. The subgoal scores for the following goals: biodiversity (A), foob provisioning (B), sense of place (C), and livelihoods and economies (D).  The scores are displayed as a map (left) and histogram (right).
...
FP <- textGrob("B Food provision", gp=gpar(fontsize=12), just="left", x=0)
fp <- textGrob("")
...
LE <- textGrob("D Livelihoods and economies", gp=gpar(fontsize=12), just="left", x=0)
le <- textGrob("")
...
##### Figure 8. Barplot of average annual change in goal trends for each region.
...
##### Figure 9. Pairwise comparison of goal scores.
...
##### Table 3. Relationship between Index scores and social variables
...
##### Figure 10. Relationship between OHI Index and social variables
...
HDI_txt <- textGrob("A", gp=gpar(fontsize=12), just="left", x=0.15, y=0)

CHI <- rasterGrob(readPNG("figures/OHIvsCHI.png"), interpolate=TRUE, y=.7)
CHI_txt <- textGrob("B", gp=gpar(fontsize=12), just="left", x=0.15, y=0)

pop <- rasterGrob(readPNG("figures/OHIvsCoastalPop.png"), interpolate=TRUE, y=.7)
pop_txt <- textGrob("C", gp=gpar(fontsize=12), just="left", x=0.15, y=0)
...
##### Figure 11. Change in score vs. rank for the 10 OHI goals
...
##### Figure 12. Change in score vs. rank for OHI subgoals
Data for subgoals of biodiversity (A), food provision (B), sense of place (C), and livelihoods and economies (D). Change is determined by subtracting the 2012 data (ranks or scores) from the 2016 data.  The changes in rank and scores are strongly correlated, but in some instances, small (probably non-significant) changes in scores can lead to large changes in rank.

```{r subgoal score vs rank, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE, fig.width=5, fig.height=10}

rank_sub <- function(goal_rank){
