...
radicalFile = '2016-10-12' #date extension on the radical data files that are used for all tables/figures
...
These datasets and figures include the complete data for the OHI 2016 assessment.

## Complete datasets (compiled for website)
...
## Figures that provide an overview of scores
This carpet plot figure ([available as high resolution png]() provides a full overview of the scores from 2016 assessment.  

Another resource that can be useful for examining scores is this [interactive plot]().  These plots can be used to (some example screen shots): 
...
print(Motion, file=sprintf('figures/GoogleVisScores.html', saveFile))

```
## More figure resources

* The location of maps describing goal and index scores are located [here]()
* Flower plots are located [here]()
...
This interactive table describes the index and goal scores for the regions in 2016 (and a [link]() to a color coded table, and a csv file can also be [downloaded]()).  
...
write.csv(data_final, sprintf("data/scores_eez%s.csv", scenario), row.names = FALSE, na='')
...
(Maps of goal scores will be presented later)
...
scores_df <- read.csv(sprintf('data/scores_eez%s.csv', scenario), stringsAsFactors = FALSE) %>%
...
for (fld in mapFlds) { # fld <- mapFlds[1]
   ...
   fig_save = sprintf('figures/maps_by_goal_%s/global_map_%s_%s_%s.png', prj, fld, scenario, prj)
...
Color-coded table of 5 year trends available [here](), and a corresponding [csv file]().  This is calculated using a linear model of scores for each region/goal over the past 5 years.  Positive values indicate potentially increasing scores during the past 5 years and negative values indicate potentially decreasing scores.
...
The global average suggests a slight (but, probably negligeable) decrease in index scores during the past five years (-0.11), but there was a good deal of variation among regions.  The following histogram describes the distribution of trends in index scores:
...
If the models and source data remains the same, these scores should be exactly the same.  Differences indicate changes in methods or source data ([described here]()).  
...
* Artisanal opportunity (biggest change: +11 points): Reference point of need layer adjusted to avoid scores being driven by outliers
...
* Fisheries (-4 points): Decrease is due primarily to increase in number of stocks classified as "unidentified" in source data

The rest of the goals/subgoals changed by less than 3 points.
...
## Comparing assessment years: regional scores

This colorcoded [dataset]() compares the 2015 index scores generated for each region and goal between this year's and last year's assessment.  

Although a goal may have had little average change, some regions may have very different scores.  

Because these are index scores, changes reflect updates to pressure and resilience scores as well as status.  

The following plot provides an overview of how the 2015 scores changed between the 2015 and 2016 assessment.
...
* Fisheries/Food provisioning: Lots of changes all over the place due to changes in source data
...
```{r goal histograms, message=FALSE, warning=FALSE, error=FALSE, echo=FALSE}
...
### Bottom 10 performers
```{bottom HAB, message=FALSE, warning=FALSE, error=FALSE, echo=FALSE}
...
```{r status years, warning=FALSE, error=FALSE, message=FALSE, echo=FALSE, eval=FALSE}
