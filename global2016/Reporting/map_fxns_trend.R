...
plot_scores <- function(rgn_df=fld_data, fld, fig_save = NULL, prj = 'gcs',
  ...
  rgn_df$val2 <- cut(rgn_df$val, col.brks, include.lowest = TRUE)
  tmp <- rev(names(table(rgn_df$val2)))
  tmp_labels <- gsub("-100", "min", tmp)
  tmp_labels <- gsub("100", " max", tmp_labels)
  
  df_plot <- ggplot(data = rgn_df, aes(x = long, y = lat, group = group, fill = val2)) +  
    theme(axis.ticks = element_blank(), axis.text = element_blank(),
          text = element_text(family = 'Helvetica', color = 'gray30', size = 12),
          plot.title = element_text(size = rel(1.5), hjust = 0, face = 'bold'),
          legend.position = ifelse(leg_on, 'right', 'none')) + 
    scale_fill_manual(values=rev(values), na.value = 'gray80', breaks = tmp, labels = tmp_labels, limits = tmp) + 
    labs(title = title, 
