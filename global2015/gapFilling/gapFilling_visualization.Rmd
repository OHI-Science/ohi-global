...
for (i in 1:length(mapCols)){ 
  ...
  plot(rgnOHI, border="grey75", add=TRUE,
       col=rev(brewer.pal(10, 'RdYlBu'))[cut(v, col.brks, labels=1:10, include.lowest=TRUE)])
  ...
  image(x = ix,
        y = c(y1, y2),
        z = matrix(1:10), col=rev(brewer.pal(10, 'RdYlBu')), add=T)
...
if(colorScheme == "new"){
  g_x$cols.goals.all = cut(g_x$score, breaks=seq(0, 100, by=10), include.lowest=TRUE, 
                       labels=rev(RColorBrewer::brewer.pal(10, 'RdYlBu'))) } else {
...
saveWidget(m, file="gapFilling.html")
...
## no real patterns that I can see
ggplot(subset(scores, goal=="Index" & dimension == "status"), aes(x=score)) +
