...
write.csv(data, "S2 Table.csv", row.names=FALSE)
...
ggplot(data, aes(y=factor(region, levels=unique((region)[order(meanIndex)])), x=goal_name, fill=score)) + 
  ...
  ylab("") + 
  xlab("")
ggsave("figures/carpetPlot_all.tiff", width=15, height=25, units="in")
...
layers <- read.csv("data_layers_gapfill.csv")

### histogram of data:
ggplot(layers, aes(x=proportion_gf, fill=component)) +
  geom_histogram() +
  labs(y="Number of datasets", x="Proportion gapfilled data") +
  scale_fill_manual(values=c("#C55F4B", "#95B8B6", "#534640")) +
  stat_bin(binwidth=0.1) +
  scatterTheme +
  theme(legend.position="none")

ggsave("figures/dataLayers_histogram.png", height=6, width=9)
