...
```{r layer download, include=FALSE}
...
scores_LE <- read.csv('../../eez2015/scores.csv') %>%
  filter(goal == 'LE') %>%
  filter(dimension %in% c('status', 'trend')) %>%
  filter(region_id != 0) %>%
  mutate(score = NA) 
...
tmp <- read.csv('scores.csv')
write.csv(rbind(tmp, scores_LE, scores_LIV, scores_ECO), 'scores.csv', row.names=FALSE, na="")
...
tmp <- read.csv('scores.csv')
##tmp <- na.omit(tmp) ## messing up LIV/ECO, not sure why this was included here....
...
```{r supragoals, warning=FALSE, message=FALSE}
...
if(length(table(scores$goal)) != 18){
  stop('There are not 18 goals/subgoals')
}
