...
## Species Diversity
There is no gap-filling for status calculations of this subgoal.  I do believe that there are many species that lack trend data (but are included in the status calculation).  I believe this would be a form of gap-filling because the species that are not included are functionally given the average trend of the species that do have trend data.  Figuring this out would involve an analysis at the raster level.  If this is worth estimating, I would have to talk to Casey about doing this. (at this point, I am assuming no gapfilling for trend).
...
## Iconic species
There is no gap-filling for status calculations of this subgoal.  I do believe that there are many species that lack trend data (but are included in the status calculation).  I believe this would be a form of gap-filling because the species that are not included are functionally given the average trend of the species that do have trend data.  I am getting everyone's opinion on whether this should be considered gap-filling.  We will say no for now.
...
There are several variables that could be gap-filled:
...
### Supragoals: BD, LE, FP, SP
Biodiversity (BD: subgoals Habitat and Species Diversity) and Sense of Place (SP: Iconic Species and Lasting Special Places) are calculating using the average of their subgoal data.  Food Provision (FP) is a weighted average of the Mariculture and Fisheries harvest.  

Livelihoods and Economies (LE) is also a supragoal (LIV and ECO), but I am ignoring this one for now due to impending changes in this goal.
...
# Food Provision: weighted mean based on relative FIS/MAR harvest
weights <- read.csv('../../eez2015/layers/fp_wildcaught_weight.csv') %>%
  mutate(MAR = 1-w_fis) %>%
...
FP <- scores %>%
  ...
  left_join(weights, by=c('region_id', 'goal')) %>%
  group_by(dimension, region_id) %>%
...
scores_supraGoals <- rbind(FP, BD, SP)
write.csv(rbind(tmp, scores_supraGoals), 'scores.csv', row.names=FALSE, na="")
...
if(length(table(scores$goal)) != 15){
  stop('There are not 15 goals/subgoals')
}
...
dimension <- unique(scores$dimension)
regions <- read.csv('../../eez2015/layers/rgn_labels.csv') %>%
  filter(type == "eez") %>%
  filter(rgn_id <= 250) %>%
  select(region_id = rgn_id)

blank <- expand.grid(goal=goals, dimension=dimension, region_id=regions$region_id)

final_scores <- blank %>%
  left_join(scores, by=c('goal', 'dimension', 'region_id')) %>%
  arrange(goal, dimension, region_id) %>%
  mutate(score = ifelse(region_id ==213, NA, score))

######################################
## Get average score for each country:
# identify supragoals
...
final_scores <- rbind(final_scores, countryScores)  
