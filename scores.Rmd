---
title: "2012-2016 Global scores using 2016 Methods"
output:
  html_document:
    toc: true
    toc_float: true
...
Scores displayed below are based on template data that were extracted from the global assessment. These scores will update when data layers are modified. SAY WHAT YEAR THIS IS FROM OR DISPLAY TABLE WITH MULTIPLE YEARS

TABLES:
- SHOW 5 years of scores 
- also add Index

The flower plots show year 2016 and petals mean X.



Scores for each goal included in an OHI assessment are calculated for multiple Regions within the overall Assessment Area, and then scores for each Region are averaged (weighted by area) to obtain a single score for the entire Assessment Area. For example, if an Assessment Area was a country, Regions could be coastal provinces or states within that country. Scores are comparable among all Regions within an Assessment Area because all are calculated with the same methods. Calculating scores for each Region allows one to discover any spatial heterogeneity across the Assessment Area and identify geographic or thematic priorities for Regional management actions.

Ocean Health Index scores are calculated for each region for each goal before being combined into an overall Index scoare . For global assessments, regions are the 
...
for (rid in regions$rgn_id){ # rid = regions$rgn_id[1]
  ...
  ## TODO for now skip Kiribati labels -- need to deal with separately
  if (!r$rgn_id %in% c(212, 148, 157)) {
  ...
  rid_score <- data_all_years %>% 
    filter(country == r$label) %>%
    select(-region_id, -country) %>%
    gather(key=goal, value=scores, -year) %>%
    spread(year, scores)
