...
HAB = function(layers, status_year){
  ...
  # if ice days are 50% of reference, this results in a zero status score
  ice_scale <- .50
  ...
  sea_ice_status <- sea_ice %>%
    ...
    mutate(status_mean_ref = (status_mean - ice_scale)/ice_scale) %>%
    data.frame()
  ...
  filter(year %in% trend_years) %>%
    filter(!is.na(status_mean_ref)) %>%
    group_by(region_id) %>%
    do(mdl = lm(status_mean_ref ~ year, data = .)) %>%
  ...
  sea_ice_status <- sea_ice_status %>%
    filter(year == status_year) %>%
    mutate(dimension = "status") %>%
    mutate(status_mean_ref = 100*status_mean_ref) %>%
    select(region_id, dimension, score = status_mean_ref)
