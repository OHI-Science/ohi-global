...
5. Be sure to push all ohiprep changes and pull possible changes from other fellows working in the repo!!
...
- @dev: used when testing new code in ohicore (used in v2023)
...
# https://github.com/OHI-Science/ohiprep_v2025/blob/main/globalprep/ao/v2024/output/ao_nind_scores.csv
```
...
source(paste0('https://raw.githubusercontent.com/OHI-Science/ohiprep_', version_year, '/refs/heads/main/workflow/R/common.R'))
```
...
Also, **Pull possible changes from other fellows working in the repo!!**

### Step 6

Update the newest layer's file location and file name in `metadata_documentation/layers_eez_base.csv` by running the function below and responding to the resulting prompts in the console. OR you can do it by hand, by updating the assessment year in the `dir` in the csv. 
...
- `eez_layers_meta_data/update_targets_with_pre_res.R` 
  ...
  - to update `eez_layers_meta_data/layers_eez_targets.csv` if layers are added to the resilience/pressures matrices in the `eez/conf` folder. 
...
# save results
write.csv(scores_all_years, here('eez/scores.csv'), na = '', row.names = FALSE)
```
...
### final commit from last year (v2024): cfddd4fdfc591551a35b7698dc7c2c18709f00af, cfddd4f
### NOTE: scenario_year indicates the years that are being compared across last year and this year. This will almost always be the previous year
### If the current year is used, there will be no data from last year to compare!!!

ohicore::score_check(
  
  file_name = "reverting ohiprep prev2025", 
  
  commit = "cfddd4f", # change this back to "previous" next time
  scenario_year = 2024,
  scenario_name = "eez", 
...
```{r eval = FALSE}
compare <- read.csv(here("eez/score_check/checking data 2_diff_data_2025-06-26.csv"))
...
p <- ggplot(dplyr::filter(compare, year==2023 & dimension=="status" & goal == "HAB"), aes(x=old_score, y=score)) +
