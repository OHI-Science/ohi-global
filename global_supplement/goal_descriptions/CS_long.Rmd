...
We focused on three coastal habitats known to provide meaningful amounts of carbon storage (Table 5.2): mangroves, seagrasses, and salt marshes [@duarte_marine_2000]. For mangroves, we used coastal mangroves that are on land or in river deltas. 
...
$$
x_{cs} = \frac { \displaystyle\sum _{ k=1 }^{ N }{ { (h }_{ k } } \times { w }_{ k }\times { A }_{ k }) }{ \displaystyle\sum _{ k=1 }^{ N }{ { (w }_{ k }\times { A }_{ k }) }  }, (Eq. 5.7)
$$
...
We employed several different methods for calculating habitat condition scores depending on the habitat of interest and available data (Table 5.2).

**Table 5.5. Carbon sequestration data**
...
### Trend
The trend for this goal is calculated using different methods for each habitat due to data availability (Table 5.2). 
