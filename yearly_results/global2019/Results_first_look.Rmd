...
dateFile = '2019-07-22' #date extension on data files used for all tables/figures
...
oldCommit = 'a5d197c1ae8b520fb8172c1da589ce4deb0f3e3a' # sha final commit for 2017 assessment
...
write.csv(ohi_final, here(sprintf("yearly_results/%s/scores_%s.csv", saveFile, Sys.Date())), row.names=FALSE)
```
...
ohi_final <- read.csv(here(sprintf("yearly_results/%s/scores_%s.csv", saveFile, dateFile)))

# User friendly version of data: region names are also added to data and full goal names.
ohi_final_formatted <- read.csv(here(sprintf("yearly_results/%s/scores_%s.csv", saveFile, dateFile))) %>%
...
write.csv(ohi_final_formatted, here(sprintf("yearly_results/%s/OHI_final_formatted_scores_%s.csv", saveFile, dateFile)), row.names=FALSE)

## create a version of data with region index scores wide
ohi_final_wide <- read.csv(here(sprintf("yearly_results/%s/OHI_final_formatted_scores_%s.csv", saveFile, dateFile))) %>%
...
write.csv(ohi_final_wide, here(sprintf("yearly_results/%s/Results/data/index_scores_wide_all_years.csv", saveFile)), row.names=FALSE)
...
write.csv(index_avg_spread, here(sprintf("yearly_results/%s/Results/data/avg_goal_scores.csv", saveFile)), row.names=FALSE)
...
data_all <- read.csv(here(sprintf("yearly_results/%s/OHI_final_formatted_scores_%s.csv", saveFile, dateFile)))
...
**will need to update this for 2019**
...
# ^ not working as of 7/22/19 - can't open the png file?
```
...
source(here("yearly_results/global2016/Reporting/map_fxns.R")) # why are we sourcing 2016 files? 
...
rgn_df <- get_rgn_df(prj = prj) # v2019: getting error here ! "cannot open data source", but map_fxns.R seems to be working fine?
