...
library(here)
setwd(file.path(here(), "yearly_results/global2019/data_check"))

compare <- read.csv(file.path(here(),"eez/score_check/fis_correct_diff_data_2019-10-23.csv")) 

library(ggplot2)
p <- ggplot(dplyr::filter(compare, year==2019 & dimension=="status" & goal == "FIS"), aes(x=old_score, y=score)) +
