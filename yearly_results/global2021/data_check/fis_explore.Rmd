...
# scores <- read_csv("../scores_2021-11-04.csv") %>%
#   filter(dimension == "score", year == 2021, region_id == 0) 
# 
# scores <- read_csv("../scores_2021-11-04.csv") %>%
#   filter(dimension == "score", year == 2021, region_id == 171, goal == "ICO") 
...
```{r}
bbmsy_gf <- read.csv('https://raw.githubusercontent.com/OHI-Science/ohiprep_v2021/gh-pages/globalprep/fis/v2021/output/fis_bbmsy_gf.csv') %>%
...
combine <- bbmsy %>% 
  ...
  left_join(rgn_deets) %>%
  select(year, rgn_nam, group, catch, perc_cmsy) %>%
  distinct() %>%
  mutate(status = ifelse(is.na(group), "no status", group)) %>%
  filter(year > 2008) 
# %>%
#   group_by(rgn_id) %>%
#   mutate(perc_cmsy = mean(perc_cmsy))
...
# +
 # geom_label(aes(label = perc_cmsy))
...
ile_bbmsy <- bbmsy %>%
  filter(rgn_id == 39, stock_id %in% c("Thunnus_albacares-51", "Katsuwonus_pelamis-51"))
```
...
It looks like *Theragra_chalcogramma-67* makes up a significant portion of the catch! Almost 22% of catch. This is a **Alaska Pollock**

```{r}
pollock_bbmsy <- bbmsy %>%
  filter(stock_id %in% c("Theragra_chalcogramma-61"))
...
Check to see the general trends for BBMSY
...
test <- lm(bbmsy~year+stock_id, data = ram_only)

test

test <- lm(bbmsy~year, data = ram_only)




test <- ram_only %>% do(model = lm(bbmsy ~ year + stock_id, data = .))
...
bbmsy <- read_csv("~/github/ohi-global/eez/layers/fis_b_bmsy.csv") %>%
  left_join(bbmsy_gf, by = c("rgn_id", "stock_id", "year", "bbmsy")) %>%
  filter(bmsy_data_source == "RAM", is.na(RAM_gapfilled))

ram_bmsy_gf <- bbmsy %>%
  group_by(stock_id) %>%
  do({
    mod <- lm(bbmsy ~ year, data=.)  
    ram_bmsy_predict <- predict(mod, newdata=.[c('year')])
    slope <- mod[["coefficients"]][["year"]]
    data.frame(., ram_bmsy_predict, slope)
  }) %>%
  ungroup()
