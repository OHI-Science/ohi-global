---
title: 'OHI 2020: Summary of results'
...
This document describes results from the 2020 global OHI assessment.  This is mostly an internal document to explore and check results.  It also creates the figures used on the website and in other documents.
...
dateFile = '2020-10-01' #date extension on data files used for all tables/figures

## General settings to control
scenario <- 2020 #identify scenario of focus (this can be changed to obtain data for other years)
benchmark = 2019  # previous year that is used for old vs. new OHI analyses
oldCommit = 'aa8ff510b6817934016115f59eac76f5360e1e60' # sha final commit for 2018 assessment
saveFile <- 'global2020' #location where files that are created are to be saved
...
[These data](https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/yearly_results/global2020/OHI_final_formatted_scores_2020-09-11.csv) are the OHI scores for the eez regions of 220 countries and territories from 2012 to 2020.
...
colors <-   c(reds, blues)
#write_csv(data.frame(colors), here("yearly_results/global2020/Results/color_palette.csv")) # do we comment this out for knitting? 
...
## save a version of 2020 maps without legend
year = scenario
...
Country index scores for 2020.  Maps describing goal and index scores can be downloaded from [here](https://github.com/OHI-Science/ohi-global/tree/draft/yearly_results/global2020/Results/figures/maps_by_goal_mol).


```{r index_map, echo=FALSE, fig.height=30, fig.width=10}
knitr::include_graphics(here(sprintf("yearly_results/%s/Results/figures/maps_by_goal_mol/global_map_Index_2020_mol.png", saveFile)))
...
hist_data_old <- read_csv(here("yearly_results/global2019/Results/data/scores_eez2019.csv")) %>%
select(region_name, Index)
...
Carpet plot figure ([download as high resolution png](https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/yearly_results/global2020/Results/figures/carpetPlot.png)) providing an overview of the scores from the 2020 assessment.  Each row represents a region, the main groupings represent goals, and within each goal there are 7 years of data.  Black regions indicate no data.
...
## Data
This interactive table describes the index and goal scores for the regions in 2020 (or, [download csv version](https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/yearly_results/global2020/Results/data/scores_eez2020.csv)): 
...
A color-coded table of trend values for each country is available [here](https://rawgit.com/OHI-Science/ohi-global/draft/yearly_results/global2020/Results/data/trends_2020.html) (as well as a [csv file](https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/yearly_results/global2020/Results/data/trends_2020.csv)).  
...
region_trends2 <- region_trends %>%
  filter(term == "scenario") %>%
  select(region_name, goal, long_goal,trend = estimate, p.value) %>%
  data.frame() 

write_csv(region_trends2, here("yearly_results/global2020/Results/data/trends_website.csv"))

plotdata <- region_trends2 %>%
  mutate(trend = ifelse(trend>1, 1, trend),
         trend = ifelse(trend< (-1), (-1), trend))
ggplot(filter(plotdata, goal=="AO"), aes(x=trend)) +
...
# Creates trend_barplot.png in yearly_results/global2020/Results/figures
```
...
## Scores
Goal/subgoal scores for the 2020 scenario.
...
knitr::include_graphics(here(sprintf("yearly_results/%s/Results/figures/maps_by_goal_mol/global_map_AO_2020_mol.png", saveFile)))
...
knitr::include_graphics(here(sprintf("yearly_results/%s/Results/figures/maps_by_goal_mol/global_map_SPP_2020_mol.png", saveFile)))
...
knitr::include_graphics(here(sprintf("yearly_results/%s/Results/figures/maps_by_goal_mol/global_map_HAB_2020_mol.png", saveFile)))
...
knitr::include_graphics(here(sprintf("yearly_results/%s/Results/figures/maps_by_goal_mol/global_map_CP_2020_mol.png", saveFile)))
...
knitr::include_graphics(here(sprintf("yearly_results/%s/Results/figures/maps_by_goal_mol/global_map_CS_2020_mol.png", saveFile)))
...
knitr::include_graphics(here(sprintf("yearly_results/%s/Results/figures/maps_by_goal_mol/global_map_CW_2020_mol.png", saveFile)))
...
knitr::include_graphics(here(sprintf("yearly_results/%s/Results/figures/maps_by_goal_mol/global_map_FIS_2020_mol.png", saveFile)))
...
knitr::include_graphics(here(sprintf("yearly_results/%s/Results/figures/maps_by_goal_mol/global_map_MAR_2020_mol.png", saveFile)))
...
knitr::include_graphics(here(sprintf("yearly_results/%s/Results/figures/maps_by_goal_mol/global_map_ICO_2020_mol.png", saveFile)))
...
knitr::include_graphics(here(sprintf("yearly_results/%s/Results/figures/maps_by_goal_mol/global_map_LSP_2020_mol.png", saveFile)))
...
knitr::include_graphics(here(sprintf("yearly_results/%s/Results/figures/maps_by_goal_mol/global_map_NP_2020_mol.png", saveFile)))
...
knitr::include_graphics(here(sprintf("yearly_results/%s/Results/figures/maps_by_goal_mol/global_map_TR_2020_mol.png", saveFile)))
...
## Trends
Annual change in goal/subgoal scores calculated using a linear regression on scores from 2012 to 2020. 
...
data <- read.csv(here(sprintf("yearly_results/%s/OHI_final_formatted_scores_%s.csv", saveFile, dateFile)), stringsAsFactors = FALSE) %>%
  rename("assess2020"=value) 

data_old <- read.csv(here(sprintf('yearly_results/global2019/OHI_final_formatted_scores_2019-11-15.csv', benchmark)), stringsAsFactors = FALSE) %>%
  dplyr::rename("assess2019"=value)
...
data_compare <- data_compare %>%
  arrange(goal) %>%
  mutate(change = assess2020 - assess2019)
...
The following interactive plot provides an overview of how the 2019 scenario scores changed between the 2018 and 2020 assessments for all goals and dimensions.
...
```{r compare assessments-plot, echo=FALSE, warning=FALSE}
  ...
  p <- ggplot2::ggplot(data_compare, aes(x=goal, y=change, color=dimension)) +
    ...
    theme_bw() + 
    labs(title="Change in 2020 scores ", y="Change in score", x="") +
