...
PlotFlower <- function(region_plot     = NA,
  ...
  conf <-  readr::read_csv(paste(scenario_folder, 'conf/goals.csv', sep="/"))
  ...
  w_fn <- list.files(path=paste(scenario_folder, "layers", sep="/"), pattern = "fp_wildcaught_weight",
                     full.names = TRUE)
  ...
  region_names_all <- bind_rows(
    ...
    read_csv(paste(scenario_folder, 'spatial/regions_list.csv', sep="/")) %>%
      dplyr::select(region_id   = rgn_id,
                    region_name = rgn_name)) %>%
    mutate(flower_png = sprintf('%s/flower_%s.pdf',
                                dir_fig_save,
