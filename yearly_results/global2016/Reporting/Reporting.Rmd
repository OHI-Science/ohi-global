---
title: 'OHI 2016: Summary of results (internal file for data checking and make map/flower plots)'
...
radical1 <- FALSE  # This updates the score data used for the Ocean Health Index website, if this is updated, 

radical2 <- FALSE # This updates the layer data used for the Data Explorer component of the Ocean Health Index website.

### code to run (change these if data is updated, otherwise takes a while to run):
update_data = FALSE           # This will update the datasets
...
predicting_future = FALSE     # This is an in-depth analysis of how well likely future state predicts future status.
                              # This is useful as a second level analysis...but not really for just examining the data.
...
```{r websites data, eval=update_data, include=FALSE}
...
## create a radical version with region names
rad <- read.csv(sprintf('../radical_%s.csv', radicalFile)) %>%
  left_join(rgn_names, by= "region_id") %>%
  mutate(country = ifelse(region_id == 0, "global average (EEZ, Antarctica, High Seas)", country)) %>%
  mutate(country = ifelse(region_id == 300, "global average (EEZ only)", country)) %>%
  select(scenario, goal, dimension, region_id, region_name=country, value)

write.csv(rad, sprintf("../radical_with_region_names_%s.csv", radicalFile), row.names=FALSE)

## create a version of data with scores wide
rad_wide <- read.csv(sprintf('../radical_%s.csv', radicalFile)) %>%
  left_join(rgn_names, by= "region_id") %>%
  mutate(country = ifelse(region_id == 0, "global average (EEZ, Antarctica, High Seas)", country)) %>%
  mutate(country = ifelse(region_id == 300, "global average (EEZ only)", country)) %>%
  filter(dimension == "score") %>%
  filter(goal == "Index") %>%
  select(scenario, region_id, region_name=country, value) %>%
  spread(scenario, value)

eez_avg <- filter(rad_wide, region_id == 300)

rad_wide <- rad_wide %>%
  filter(region_id != 300)

rad_wide <- rbind(eez_avg, rad_wide)

write.csv(rad_wide, sprintf("data/all_years/Index_all_years_%s.csv", radicalFile), row.names=FALSE)

### create a version of the data that averages all 5 years of scores
### so the best/worst overall can be identified
index_avg <- read.csv(sprintf('../radical_%s.csv', radicalFile)) %>%
  left_join(rgn_names, by= "region_id") %>%
  mutate(country = ifelse(region_id == 0, "global average (EEZ, Antarctica, High Seas)", country)) %>%
  mutate(country = ifelse(region_id == 300, "global average (EEZ only)", country)) %>%
  filter(dimension == "score") %>%
  group_by(goal, region_id, country) %>%
...
write.csv(index_avg_spread, sprintf("data/avg_5_year_scores_%s.csv", radicalFile), row.names=FALSE)
...
order_rgn <- data %>%
  select(country, meanIndex) %>%
  unique() %>%
  arrange(meanIndex)

data$goal <- factor(data$goal, levels = goals)
data$country <- factor(data$country, levels=order_rgn$country)
...
ggplot(data, aes(y=country, x=scenario, fill=value)) + 
  ...
  ylab("") + 
  xlab("")
ggsave(sprintf("figures/carpetPlot_%s.png", scenario), width=20, height=35)
...
Although, the overall index score did not statistically significantly change over time, several goals did have significant changes (based on a regression analysis of the average scores for each goal, although this should be considered a preliminary analysis).  Here are some conclusions from this analysis (p values < 0.05 are considered to be statistically significant - i.e., "real" - effects):
...
results_lm <- tidy(data_lm, mdl) %>%
  ...
  select(goal=long_goal, average_change_per_year = estimate, p.value) %>%
  data.frame()
...
## save a version of 2016 maps without legend
year=2016
scores_df <- read.csv(sprintf('data/scores_eez%s.csv', year), stringsAsFactors = FALSE) %>%
  rename(rgn_name = country, rgn_id = region_id) %>%
  mutate(rgn_id = as.character(rgn_id))
...
for (fld in mapFlds) { # fld <- mapFlds[1]
   ...
   fig_save = sprintf('figures/maps_by_goal_%s/year_2016_nolegend/global_map_%s_%s_%s.png', prj, fld, year, prj)
...
for (fld in mapFlds) { # fld <- mapFlds[1]
    ...
    fig_save = sprintf('figures/maps_trends/trends_map_%s_%s_%s.png', fld, year, prj)
...
The global average decreased by 0.14 points per year, although this is may not be statistically significant.  There was a good deal of variation in trend among regions.  The following histogram describes the distribution of trend (e.g., change in score per year) in index scores:
...
To obtain a more complete picture of which regions are doing well and which are doing poorly we compared the average index scores (averaged over 2012 to 2016) and the trends for each region.
...
```{r trend vs avg index, message=FALSE, warning=FALSE, error=FALSE, echo=FALSE, eval=TRUE}

index <- read.csv(sprintf("data/avg_5_year_scores_%s.csv", radicalFile)) %>%
  select(region_id, country, index_avg = Index)
...
avg_index <- index %>%
  ...
  p <- ggplot2::ggplot(trend, aes(x=index_avg, y=index_trend)) +
    ...
    theme_bw() + 
    labs(title="Index scores", y="Trend (change in score per year)", x="Average (2012 to 2016 scores)") 
...
```{r child = 'Reporting_predicting_future_state.Rmd', eval=predicting_future}
