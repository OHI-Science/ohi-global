...
scores <- read.csv(sprintf("../radical_%s.csv", dataDate)) %>%
  filter(scenario == 2016) %>%
  filter(region_id <= 250) %>%
  filter(region_id != 0) %>%
  filter(dimension == "score") %>%
  select(goal, rgn_id=region_id, score=value)
...
area <- read.csv('../../eez2016/layers/rgn_area.csv')
...
write.csv(data, sprintf('../EcoregionSummary/OHI_scores_UN_georgn_summary_%s.csv', dataDate), row.names=FALSE)
...
## read in map data:
p <- readOGR(dsn=file.path(dir_M, "git-annex/globalprep/spatial/d2014/data"), layer="regions_mol")

## This isolates the main map parts: 
land  <-  p[p@data$ant_typ %in% c("land", "land-ccamlr", "land-disputed", "land-noeez"),]
ocean  <-  p[p@data$ant_typ %in% c("eez-inland", "fao", "eez-ccamlr"),]
rgnOHI <-  p[p@data$ant_typ %in% c("eez", "eez-disputed"),]

PlotData <- data %>%
  filter(rgn_id <= 250) %>%
  filter(rgn_id != 0) %>%
  dplyr::select(code=rgn_id, goal, score=score_weighted_mean) %>%
  left_join(goal_names) %>%
  mutate(goal = factor(goal_name, levels=goal_names$goal_name)) %>%
  dplyr::select(-goal_name)
...
for (i in 1:length(mapCols)){ 
  ...
  fig = sprintf('../EcoregionSummary/rgnMaps/global_map_mol_%s.png', fld)
  ...
  plot(rgnOHI, border=MyColor[cut(v, col.brks, labels=1:10, include.lowest=TRUE)], add=TRUE,
