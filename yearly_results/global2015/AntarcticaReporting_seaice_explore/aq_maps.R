...
data <- read.csv('antarctica2015_seaice_explore/scores.csv') %>%
  filter(region_id != 0) %>%
...
ggsave(sprintf('global2015/AntarcticaReporting_seaice_explore/maps/%s.png', goals[which(names(goals)==goal)]), width=8, height=6)
}
