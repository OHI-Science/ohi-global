...
update_data = FALSE # dplyr::select true if there are updates to the OHI data!! Also be sure to change dateFile above
...
# ideally we use the same color palatte 
...
# Define standard color palette for histograms
...
readr::read_csv(current_scores_fn) %>%
...
# Consider deleting below in favor of above


```{r goal output functions, message=FALSE, warning=FALSE, error=FALSE, echo=FALSE}
## Define helper functions for following outputs
...
print_plots <- function(goal = "AO") { 
  p1 <- here::here(dir_goal_maps, paste0("global_map_",goal,"_",scenario,"_mol.png"))
  p2 <- here::here(dir_goal_histograms, paste0(goal,"_scores.png"))
...
```{r AO plots, echo=FALSE, fig.height=30, fig.width=10, results='asis'}
print_plots(goal = "AO")
```
...
p <- ggplot2::ggplot(trend, aes(x=index_avg, y=index_trend)) +
  ...
  # geom_vline(xintercept = avg_index, color="red", size=.5, linetype=3, aes(text = "avg index")) +
