...
Index_map <- here::here(dir_goal_maps, paste0("global_map_Index_", scenario,"_mol.png")) %>% 
  png::readPNG() %>% 
  grid::rasterGrob(interpolate=TRUE)

map_label <- grid::textGrob("", just = 19, gp = gpar(fontsize=18))

Index_hist <- here::here(dir_results_figures, "score_histogram_color.png") %>% 
  png::readPNG() %>% 
  grid::rasterGrob(interpolate=TRUE)

hist_label <- grid::textGrob("", just = 19, gp = gpar(fontsize=18))

gridExtra::grid.arrange(map_label, Index_map, hist_label, Index_hist,
...
text <- textGrob("", gp = gpar(fontsize = 14), vjust = -9.5)

goal_map <- function(goal) {
  here::here(dir_map_no_leg, paste0("global_map_",goal,"_",scenario,"_mol.png")) %>% 
    png::readPNG() %>% 
    grid::rasterGrob(interpolate = TRUE)
}

goal_hist <- function(goal) {
