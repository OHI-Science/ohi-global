...
get_rgn_df <- function(
    dir =  here::here('eez', 'spatial', 'downres'),
    layer = 'rgn_eez_mol_med_res', prj = 'mol') {

  rgn_shp <- terra::vect(x = dir, layer = layer, crs = 'ESRI:54009')

  rgn_df <- base::data.frame(rgn_shp)
...
ohi_regions <- terra::vect(file.path("/home/shares/ohi/git-annex/globalprep/spatial/v2017")) %>% 
  dplyr::filter(rgn_type =="eez", !rgn_name %in% c("Antarctica", "DISPUTED"))

ohi_regions_2 <- terra::rescale(ohi_regions, 5000)

ggplot() +
  tidyterra::geom_spatvector(data = world, fill = NA, color = "red") +
  tidyterra::geom_spatvector(data = ohi_regions_2, fill = NA, alpha = .1)
