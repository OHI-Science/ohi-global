...
dateFile <- "2022-08-29" # date extension on data files used for all tables/figures
benchmark_date <- "2021-11-17" # Date from last assesment
```
...
map.col.brks <- seq(0, 100, length.out = 6)
...
```{r Maps_v2022, eval=map_create, echo=FALSE, error=FALSE, message=FALSE, warning=FALSE}
### This used to be a long and difficult to follow function from the times before
### geom_sf or tidyterra. This section was updated in v2022 with the goal of 
### simplifying the process and making it easier to follow and edit in the future 
### but it could be made into a function and placed in the functions folder again
### This would help for the no legend vs legend versions.
### I would also suggest parallelizing the first for loop and do all years at once
  ...
  for (i in seq_along(goals)) { # i = 1
    ...
    p1 <- ggplot() +
      ...
      scale_fill_gradientn(colours = colors, space = 'Lab', na.value = 'gray80',
                           breaks = map.col.brks, labels = map.col.brks, limits = c(0, 100)) +
...
```{r Maps_no_leg, eval=map_create, echo=FALSE, error=FALSE, message=FALSE, warning=FALSE}
### save a version of latest scenario year maps without legend
### This is where a function could help (see above comment)
year = scenario
...
### This could be cleaned up just  like the maps above...
