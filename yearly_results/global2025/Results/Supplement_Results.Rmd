...
### See ohi-global/yearly_results/global2024/Results_first_look.Rmd 
...
read_csv("https://raw.githubusercontent.com/OHI-Science/OHI-website/refs/heads/main/tables/ten_goals.csv", col_types = cols()) %>% 
  knitr::kable()
```
...
readr::read_csv("https://raw.githubusercontent.com/OHI-Science/OHI-website/refs/heads/main/tables/Subgoal_Descriptions.csv", col_types = cols()) %>% 
  knitr::kable()
```
...
A CSV formatted dataset from the 2025 assessment is [available](https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/yearly_results/global2025/OHI_final_formatted_scores_2025-09-11.csv), right click on data and select "Save as...". These data include index and goal/subgoal scores (as well as the dimensions used to calculate scores: status, trend, pressure, resilience) for all `r 2025 - 2011` assessment years (2012-2025). See [README](https://github.com/OHI-Science/ohi-global/tree/draft/yearly_results/global2025) for description of data.

The data layers (described in section 7 in Methods) and functions used to calculate scores can be downloaded from Github: [ohi-global v2024.1: Global scenarios data for Ocean Health Index](https://github.com/OHI-Science/ohi-global/releases).  

The files used to prepare data layers for the ohi-global assessment can be downloaded from Github: [ohiprep v2024.1: Preparation of data for 2024 Ocean Health Index global assessment](https://github.com/OHI-Science/ohiprep_v2024/releases).
...
##### Figure A. Regional 2025 OHI index scores.

Map (top) and histogram (bottom) of 2025 index scores for each OHI region. 
...
##### Figure B. Flowerplot of 2025 goal/subgoal scores

Flower plot describing the 2025 average global index (middle) and goal (petals) scores. Scores are an eez area weighted average of regional scores.  
...
##### Figure C. Maps and histograms of 2025 OHI goal scores.

The 2025 scores for each of the 10 OHI goals displayed for each region (left) and as a histogram (right).
...
##### Figure D. Maps and histograms of 2025 OHI subgoal scores.

The subgoal 2025 scores for the following goals: biodiversity, foob provision, sense of place, and livelihoods and economies.  The scores are displayed as a map (left) and histogram (right).
...
Overview of the scores from the 2025 assessment. Each row represents a region, the main groupings represent goals, and within each goal there are `r 2025 - 2011` years of data. Black regions indicate no data. Download the [png version](https://github.com/OHI-Science/ohi-global/raw/draft/yearly_results/global2025/Results/figures/carpetPlot.png) to zoom in to see specific regions.
...
Map (top) and histogram (bottom) of the slope estimates from a linear regression model of the Index scores from 2012 to 2025 for each region.
...
Map of the slope estimates from a linear regression model of the 10 goal scores from 2012 to 2025 for each region.
...
Map of the slope estimates from a linear regression model of the 8 subgoal scores from 2012 to 2025 for each region. The subgoal scores for the following goals: biodiversity, food provision, sense of place, and livelihoods and economies.  The scores are displayed as a map (left) and histogram (right).
...
The values are the slope estimate from a linear regression model of the scores from 2012 to 2025 for each region and goal.
