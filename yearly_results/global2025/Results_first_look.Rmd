...
###### update this 2024!######
dateFile       <- "2025-11-26" # date extension on data files used for all tables/figures
...
[These data](https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/yearly_results/global2025/OHI_final_formatted_scores_2025-11-26.csv) are the OHI scores for the eez regions of 220 countries and territories from 2012 to 2025.
...
trends_wide <- rbind(index, eez_wide) %>%
 dplyr::select(country, region_id, !!!rlang::syms(grep("ECO", goals, invert=TRUE, value=TRUE))) %>% 
...
for(goal in grep("ECO", goals, invert=TRUE, value=TRUE)){ #goal="CW"
...
for(goal in grep("ECO", goals, invert=TRUE, value=TRUE)){ # goal = "HAB"
