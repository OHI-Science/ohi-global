...
This document describes results from the 2025 global OHI assessment. This is mostly an internal document to explore and check results. It also creates the figures used on the website and in other documents.

Note for 2025 fellows- I updated most of the notes, values, and code that required years, copied over folders, and ran most of the script but there are some places I didnt change which I marked with ###### update this 2024!######. These are mostly github links to download from and I think all the 2024 scores need to be updated and pushed before we can change that-Sanj
...
scenario   <- 2025 # identify scenario of focus (this can be changed to obtain data for other years)
benchmark  <- 2024 # previous year that is used for old vs. new OHI analyses
globalYear <- paste0('global', scenario) # location where files that are created are to be saved

#you can find this at https://github.com/OHI-Science/ohi-global/commits/draft/yearly_results/global2024 after you change the year
oldCommit <- 'cfddd4fdfc591551a35b7698dc7c2c18709f00af' #final commit for 2024
### 'e7497d697c654c7d5c49b76cb70eb22b31a61e13' # final commit for 2023 assessment
...
###### update this 2024!######
dateFile       <- "2025-09-11" # date extension on data files used for all tables/figures
benchmark_date <- "2024-09-11" # Date from last assessment
```
...
[These data](https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/yearly_results/global2024/OHI_final_formatted_scores_2024-09-11.csv) are the OHI scores for the eez regions of 220 countries and territories from 2012 to 2025.
...
###### update this 2025!######
Flower plots for each region can be downloaded from [here](https://github.com/OHI-Science/ohi-global/tree/draft/yearly_results/global2025/Results/figures/FlowerPlots)
...
Country index scores for 2025  Maps describing goal and index scores can be downloaded from [here](https://github.com/OHI-Science/ohi-global/tree/draft/yearly_results/global2025/Results/figures/maps_by_goal_mol).
...
###### update this 2024!######
Carpet plot figure ([download as high resolution png](https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/yearly_results/global2025/Results/figures/carpetPlot.png)) providing an overview of the scores from the 2024 assessment.  Each row represents a region, the main groupings represent goals, and within each goal there are 12 years of data. Black regions indicate no data.
...
This interactive table describes the index and goal scores for the regions in 2025 (or, [download csv version](https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/yearly_results/global2025/Results/data/scores_eez_2025.csv)): 
...
final_filename %>% 
  ...
  dplyr::summarise(change = round(`2025` - `2012`, 3)) %>% 
...
A color-coded table of trend values for each country is available [here](https://rawgit.com/OHI-Science/ohi-global/draft/yearly_results/global2025/Results/data/trends_2025.html) (as well as a [csv file](https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/yearly_results/global2025/Results/data/trends_2025.csv)).  
...
This section takes a closer look at the goal/subgoal scores for the 2025 scenario.
...
To obtain a more complete picture of which regions are doing well and which are doing poorly we compared the average index scores (averaged over 2012 to 2025) and the trends for each region.
...
The following is a comparison of the global status scores generated for the 2025 scenario by this year's assessment vs. last year's assessment. 
...
data_compare <- data_compare %>%
  dplyr::arrange(goal) %>%
  dplyr::mutate(change = assess2025 - assess2024)
...
The following interactive plot provides an overview of how the 2024 scenario scores changed between the 2024 and 2025 assessments for all goals and dimensions.
...
p <- ggplot2::ggplot(data_compare, aes(x = goal, y = change, color = dimension)) +
  ...
  theme_bw() + 
  labs(title = "Change in 2025 scores ", y = "Change in score", x = "") +
