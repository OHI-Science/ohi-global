...
This document describes results from the 2024 global OHI assessment. This is mostly an internal document to explore and check results. It also creates the figures used on the website and in other documents.

Note for 2024 fellows- I updated most of the notes, values, and code that required years, copied over folders, and ran most of the script but there are some places I didnt change which I marked with ###### update this 2024!######. These are mostly github links to download from and I think all the 2024 scores need to be updated and pushed before we can change that-Sanj
...
scenario   <- 2024 # identify scenario of focus (this can be changed to obtain data for other years)
benchmark  <- 2023 # previous year that is used for old vs. new OHI analyses
globalYear <- paste0('global', scenario) # location where files that are created are to be saved

#you can find this at https://github.com/OHI-Science/ohi-global/commits/draft/yearly_results/global2023 after you change the year
oldCommit  <- 'e7497d697c654c7d5c49b76cb70eb22b31a61e13' # final commit for 2023 assessment
### '33ca88c7f4cd7cbcd381bdf0921f4d553f1e344d' # final commit for 2022 assessment
### 'af4b1f3baf55704f272738d900560fb92cb81ab1' # sha final commit for 2021 assessment
...
update_data  <- TRUE # select true if there are updates to the OHI data!! Also be sure to change dateFile below
...
###### update this 2024!######
dateFile       <- "2024-09-11" # date extension on data files used for all tables/figures
benchmark_date <- "2023-10-02" # Date from last assessment
```
...
#this shows an error message at first but if you run it again it should work..
  data <- data_all %>%
...
[These data](https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/yearly_results/global2024/OHI_final_formatted_scores_2024-09-11.csv) are the OHI scores for the eez regions of 220 countries and territories from 2012 to 2024.
...
###### update this 2024!######
Flower plots for each region can be downloaded from [here](https://github.com/OHI-Science/ohi-global/tree/draft/yearly_results/global2023/Results/figures/FlowerPlots)
...
Country index scores for 2024  Maps describing goal and index scores can be downloaded from [here](https://github.com/OHI-Science/ohi-global/tree/draft/yearly_results/global2024/Results/figures/maps_by_goal_mol).
...
###### update this 2024!######
Carpet plot figure ([download as high resolution png](https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/yearly_results/global2024/Results/figures/carpetPlot.png)) providing an overview of the scores from the 2024 assessment.  Each row represents a region, the main groupings represent goals, and within each goal there are 12 years of data. Black regions indicate no data.
...
This interactive table describes the index and goal scores for the regions in 2024 (or, [download csv version](https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/yearly_results/global2024/Results/data/scores_eez_2024.csv)): 
...
## Change from 2012 to 2024

This interactive table describes the gross change in index and goal scores for the regions between 2012 and 2024. This is helpful in investigating a single country by searching the region name and seeing how much each goal value has changed overall from the first year to the last.  
...
final_filename %>% 
  ...
  dplyr::summarise(change = round(`2024` - `2012`, 3)) %>% 
...
A color-coded table of trend values for each country is available [here](https://rawgit.com/OHI-Science/ohi-global/draft/yearly_results/global2024/Results/data/trends_2024.html) (as well as a [csv file](https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/yearly_results/global2024/Results/data/trends_2024.csv)).  

NOTE: Currently, these data include the livelihoods and economies goal but these results should be viewed skeptically because these data were last updated for the 2013 assessment.  Many other goals contain components with poor data resolution (i.e., infrequently, or never, measured over time).  
...
write.csv(data_wide, file = paste0(dir_results_data, "/area_weighted_average_scores_by_year_and_goal_v", scenario,".csv"), row.names = FALSE)
```
...
# Creates trend_barplot.png in yearly_results/global2023/Results/figures when called
...
This section takes a closer look at the goal/subgoal scores for the 2024 scenario.
...
Annual change in goal/subgoal scores calculated using a linear regression on scores from 2012 to 2024. 
...
To obtain a more complete picture of which regions are doing well and which are doing poorly we compared the average index scores (averaged over 2012 to 2024) and the trends for each region.
...
The following is a comparison of the global status scores generated for the 2024 scenario by this year's assessment vs. last year's assessment. 
...
data_compare <- data_compare %>%
  dplyr::arrange(goal) %>%
  dplyr::mutate(change = assess2024 - assess2023)
...
The following interactive plot provides an overview of how the 2023 scenario scores changed between the 2023 and 2024 assessments for all goals and dimensions.
...
p <- ggplot2::ggplot(data_compare, aes(x = goal, y = change, color = dimension)) +
  ...
  theme_bw() + 
  labs(title = "Change in 2024 scores ", y = "Change in score", x = "") +
  scale_x_discrete(limits = c("Index", "AO", "SPP", "BD", "HAB", "CP", "CS",
                              "CW", "FIS", "FP", "MAR", "ECO", "LE", "LIV",
                              "NP", "LSP", "SP", "ICO", "TR")) +
