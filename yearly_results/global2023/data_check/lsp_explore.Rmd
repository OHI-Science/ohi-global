...
lsp_scores <- read_csv(file.path(here(), "yearly_results/global2023/scores_2023-10-02.csv"))
...
```{r}
prot_3nm <- read.csv('https://raw.githubusercontent.com/OHI-Science/ohiprep_v2023/gh-pages/globalprep/lsp/v2023/int/area_protected_3nm.csv')

prot_3nm_2022 <- prot_3nm %>%
  filter(year == 2022) %>%
  mutate(type = "3nm")

sum(prot_3nm_2022$a_prot_km2) # [1] 1106411
sum(prot_3nm_2022$a_tot_km2) # 4995189

1106411/4995189 # 0.2214953 ; ~22% of 3nm areas are protected 


prot_1km <- read.csv('https://raw.githubusercontent.com/OHI-Science/ohiprep_v2023/gh-pages/globalprep/lsp/v2023/int/area_protected_1km.csv')

prot_1km_2022 <- prot_1km %>%
  filter(year == 2022) %>%
  mutate(type = "1km")

sum(prot_1km_2022$a_prot_km2) # 302110.8
sum(prot_1km_2022$a_tot_km2) # 1133638

302110.8/1133638 # 0.2664967 ; ~27% of 1km areas are protected 


prot_eez <- read.csv('https://raw.githubusercontent.com/OHI-Science/ohiprep_v2023/gh-pages/globalprep/lsp/v2023/int/area_protected_eez.csv')

prot_eez_2022 <- prot_eez %>%
  filter(year == 2022) %>%
  mutate(type = "eez")

sum(prot_eez_2022$a_prot_km2) # 28099489
sum(prot_eez_2022$a_tot_km2) # 359482664

28099489/359482664 # 0.07816646 ; ~7.7% of eez areas are protected 


all_prot <- rbind(prot_eez_2022, prot_1km_2022, prot_3nm_2022) %>%
  group_by(type) %>%
