...
This document describes results from the 2023 global OHI assessment. This is mostly an internal document to explore and check results. It also creates the figures used on the website and in other documents.
...
scenario   <- 2023 # identify scenario of focus (this can be changed to obtain data for other years)
benchmark  <- 2022 # previous year that is used for old vs. new OHI analyses
globalYear <- paste0('global', scenario) # location where files that are created are to be saved
oldCommit  <- '33ca88c7f4cd7cbcd381bdf0921f4d553f1e344d' # final commit for 2022 assessment
...
This interactive table describes the index and goal scores for the regions in 2023 (or, [download csv version](https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/yearly_results/global2023/Results/data/scores_eez_2023.csv)): 
...
## Change from 2012 to 2023

This interactive table describes the gross change in index and goal scores for the regions between 2012 and 2023. This is helpful in investigating a single country by searching the region name and seeing how much each goal value has changed overall from the first year to the last.  
...
final_filename %>% 
  ...
  dplyr::summarise(change = round(`2023` - `2012`, 3)) %>% 
...
p <- ggplot2::ggplot(data_compare, aes(x=goal, y=change, color=dimension)) +
  ...
  theme_bw() + 
  labs(title="Change in 2023 scores ", y="Change in score", x="") +
