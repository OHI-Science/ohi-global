---
title: 'Ocean Health Index 2023: Methods'
...
#5. If there are changes to the references, run BIBcorrect.R!!!
  # 6. Review document to fix year references (i.e., title at top, assessmentYear variable in this code chunk, update figure 4.1)
 # 7. Knit this file and review html output. 

## {{< redirect path="https://ohi-science.org/ohiprep_v2024/globalprep/ao/v2023/ao_access_data_prep.html" >}}
...
```
# Return to [main website](https://oceanhealthindex.org)
...
# Selecting data and missing data
...
# function
data_layers <- function(dimension, goal){ # goal = "MAR"; dimension=c("status", "trend")
...
url <- read.csv(here("metadata_documentation/layers_eez_dataprep.csv")) %>%
  ...
  mutate(count = sum(N)) %>%
  mutate(url_link = ifelse(count==1,  sprintf("[data prep](https://ohi-science.org/ohiprep_v%s/%s)", assessmentYear, dataprep_url), 
                sprintf("[data prep %s](https://ohi-science.org/ohiprep_v%s/%s)", method_count, assessmentYear, dataprep_url))) %>%
