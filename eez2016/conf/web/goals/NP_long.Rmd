...
To create a single score for the Natural Products goal ($x_{NP}$) we then took the weighted average of the individual product scores, such that:

$$
{x}_{NP} = \frac { \sum _{ p=1 }^{ N }{ { w }_{ p }\ast x_{ p } }  }{ { N } }
$$

where $_{N}$ is the number of products that have ever been harvested and $w_{p}$ is the relative contribution of each product to the overall status of the goal, which is the ratio of the maximum US dollar value for a product (from the smoothed, gap-filled data) across all years of data for the product, relative to the sum of maximum values for all products harvested in the country.

<!-- If a product had a peak value, but was missing a harvest value for that product in a given year, then we used $w_{p}$ = 0 during the aggregation for that year.  -->

Note that for the Trend calculation, **some regions have staggered years between products for the slope calculations (2003-2007 for Corals, Ornamental Fish, and Shells and 2004-2008 for the rest) --> change the years**.
...
**Table 6. Definitions and weights assigned for each category of exploitation status**
